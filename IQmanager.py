...
from myfxbook_scraper import volatility 
...
#%%
        while True:
            ...
            if connector.get_positions(instrument_type)[1].get('total') > 1:
                time.sleep(60*3)
            else:
                break
...
#%%
        ...
        leverage=min(connector.get_available_leverages(instrument_type,instrument_id)[1].get('leverages')[0].get('regulated'))
        amount = (balance*0.95) /leverage if (balance*0.95) /leverage < 20000 else 20000
