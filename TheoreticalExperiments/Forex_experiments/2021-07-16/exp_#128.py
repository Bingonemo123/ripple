    ...
    try:
        ...
        for  f in  ALL_Asset['forex'].keys():
            ...
            for ps in past_time_assets_diff_trade:
                if ps[1] <  diff/(2*mean):
                    past_time_assets_diff_trade =  past_time_assets_diff_trade[:count] + [ diff/(2*mean)] + past_time_assets_diff_trade[count:]
                    break
                count += 1

            else:
                past_time_assets_diff_trade.append(diff/(2*mean))
            print(f, date, diff/(2*mean))
            print(past_time_assets_diff_trade)
