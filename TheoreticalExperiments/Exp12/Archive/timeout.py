...
def custom_volmeter(connector, f):
    if connector.market_book_add(f):
        ...
        return f"mt5.market_book_add({f}) failed, error code = {connector.last_error()}"


@timeout(120)
def datamine(connector, f, frame, t, count):
    return connector.copy_rates_from(f, frame, t, count)