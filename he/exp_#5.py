"""
    ...
    In this Experiment, procent volatility and standard deviation volatility will be 
    measured and compared to volatility n1

    date: 06/07/2021
    method: volatility percentage divisor max
    ['NZDUSD', 'NZDJPY', 'AUDUSD', 'NZDCAD', 'AUDJPY', 'NZDCHF', 'EURNZD', 'CADJPY', 'AUDCAD', 'AUDCHF', 'USDCAD', 'EURAUD', 'CADCHF', 'GBPCAD', 'EURCAD', 'GBPJPY', 'AUDNZD', 'GBPUSD', 'CHFJPY', 'USDCHF', 'EURJPY', 'EURUSD', 'EURGBP', 'USDJPY', 'EURCHF']

    date: 06/07/2021
    method: volatility percentage divisor min
    ['NZDUSD', 'NZDJPY', 'AUDUSD', 'NZDCAD', 'AUDJPY', 'NZDCHF', 'EURNZD', 'CADJPY', 'AUDCAD', 'AUDCHF', 'USDCAD', 'EURAUD', 'CADCHF', 'GBPCAD', 'EURCAD', 'GBPJPY', 'AUDNZD', 'GBPUSD', 'CHFJPY', 'USDCHF', 'EURJPY', 'EURUSD', 'EURGBP', 'USDJPY', 'EURCHF']

    date: 06/07/2021
    method: standard deviation
    note: this list is very similar to volatility N1 method list. maybe it also needs reduction to same values, like percentages.
    ['GBPJPY', 'EURJPY', 'CHFJPY', 'USDJPY', 'CADJPY', 'AUDJPY', 'NZDJPY', 'GBPCAD', 'EURNZD', 'EURAUD', 'EURCAD', 'GBPUSD', 'USDCAD', 'EURUSD', 'EURCHF', 'AUDNZD', 'AUDCAD', 'USDCHF', 'NZDCAD', 'EURGBP', 'AUDUSD', 'CADCHF', 'NZDUSD', 'AUDCHF', 'NZDCHF']
    

    date: 06/07/2021
    method: percentages with standard deviation
    ['NZDUSD', 'NZDJPY', 'AUDUSD', 'CADJPY', 'AUDJPY', 'EURNZD', 'USDCAD', 'NZDCHF', 'NZDCAD', 'CADCHF', 'AUDCAD', 'EURCAD', 'EURAUD', 'AUDCHF', 'GBPCAD', 'AUDNZD', 'GBPJPY', 'GBPUSD', 'USDCHF', 'CHFJPY', 'EURJPY', 'EURUSD', 'EURGBP', 'USDJPY', 'EURCHF']
...
if __name__ == '__main__':
    try:
        ...
        date = datetime.date(year= 2021, month=7, day=7)  # day must be + 1,beacuse this is end date
        ...
        for f in tqdm(ALL_Asset['forex'].keys()):
            ...
            per_l = []
            for candle in data:
                per_l.append((candle.get("max")-candle.get("min"))/candle.get('max'))

            mean = sum(per_l)/len(per_l)
            
            deviation = []
            for el in per_l:
                deviation.append(el - mean) 

            sqdev = [x**2 for x in deviation]

            varianve = sum(sqdev)/len(sqdev)
            standart_deviation = varianve**(0.5)
            volatility = standart_deviation
