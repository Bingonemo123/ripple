...
Purpuse of this experiment is, to compare max or min leverage. In previus experiment 
was showned with max leverage selected that total position closed: 70. In this
Experiment in would be changed to min. No other changes would be done.

Experiment showed that after changed leverage total positions closed (tpc) dropped
to 63. So may be concluded insignificant change.
"""
import datetime
import os
import shutil
...
shutil.copy(file_path, path / (file_path.stem +
            str(experiment_number) + file_path.suffix))
...
"""FileHandler"""
rotatingfile_handler = logging.handlers.RotatingFileHandler(
    path.parent/'main.log', backupCount=5, maxBytes=1073741824)
    ...
    try:
        ...
        for f in ALL_Asset['forex'].keys():
            ...
            for ps in past_time_assets_diff_trade:
                if ps[1] < diff/(2*mean):
                    past_time_assets_diff_trade = past_time_assets_diff_trade[:count] + [
                        (f, diff/(2*mean))] + past_time_assets_diff_trade[count:]
                    break
                count += 1
        ...
        def past_history(f, date=end_from_time):
            ...
            leverage = min(connector.get_available_leverages('forex', f)[
                           1].get('leverages')[0].get('regulated'))
        ...
        for mt in range(1440):
            ...
            for f in buyed_fs:
                ...
                leverage = max(connector.get_available_leverages('forex', f)[
                               1].get('leverages')[0].get('regulated'))
                curpr = connector.get_candles(f, 60, 1, end_from_time + mt*60)
